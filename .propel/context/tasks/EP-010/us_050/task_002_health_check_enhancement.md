...
- [ ] Create aiServiceHealthCheck.ts with checkAIServiceHealth function (test OpenAI API /v1/models endpoint with 5s timeout, return available/latency/error)
- [ ] Modify app.ts to enhance GET /api/health endpoint with AI service check (import checkAIServiceHealth, add aiService to response)
- [ ] Update /api/health HTTP status logic to return 503 if database OR Redis OR AI service fails (isHealthy = dbStatus.ok && redisStatus.connected && aiServiceStatus.available)
- [ ] Add execution time tracking to health check endpoint (const startTime = Date.now(), calculate executionTime at end)
- [ ] Write unit tests for checkAIServiceHealth with mocked axios (test success, timeout, auth error scenarios)
- [ ] Write integration tests for /api/health endpoint (test with healthy dependencies, test with each dependency failing)
- [ ] Test health check manually (curl http://localhost:3000/api/health, verify 200 when healthy, verify 503 when dependency fails)
- [ ] Test PM2 reload with health check monitoring (pm2 reload upaci-backend while polling /api/health in loop, verify no 503 errors during reload)
