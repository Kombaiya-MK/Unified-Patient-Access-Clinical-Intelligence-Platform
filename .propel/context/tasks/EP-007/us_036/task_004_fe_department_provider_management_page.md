...
- [ ] Create app/src/types/department.ts (interfaces: Department with id/name/code/description/operating_hours object with day keys/location/phone_number/email/is_active/provider_count/appointment_count/created_at, CreateDepartmentInput, UpdateDepartmentInput)
- [ ] Create app/src/types/provider.ts (Provider with id/user_id/specialty/license_number/first_name/last_name/email/phone_number/departments string/total_weekly_hours/is_active, ScheduleEntry with id/day_of_week/start_time/end_time/is_available, BlockedTime with id/blocked_date/start_time/end_time/reason, CreateProviderInput, UpdateProviderScheduleInput)
- [ ] Create app/src/hooks/useDepartments.ts (useQuery GET /api/admin/departments with page/filterStatus, refetchInterval 10000 for NFR-REL01, useMutation createDepartment POST, useMutation updateDepartment PUT, useMutation deactivateDepartment PATCH with error handling for 409 future appointments, return departments/isLoading/error/pagination/mutations)
- [ ] Create app/src/hooks/useProviders.ts (useQuery GET /api/admin/providers with filterDepartment/filterSpecialty/filterStatus refetchInterval 10000, useMutation createProvider POST, useMutation updateProvider PUT, useProviderSchedule GET /providers/:id/schedule, useMutation updateProviderSchedule POST with error handling for 400 overlap, useMutation createBlockedTime POST with error handling for 409 conflicts, useMutation deleteProvider DELETE with error handling for 409 future appointments, useProviderAppointments GET /providers/:id/appointments)
- [ ] Create app/src/components/admin/OperatingHoursGrid.tsx (accept value/onChange from React Hook Form useController, render 7-row grid for Monday-Sunday, each row: day label, is_open checkbox, open time picker input type="time" disabled if not is_open, close time picker disabled if not is_open, validate open < close with error message, return operating_hours JSON object structure)
- [ ] Create app/src/components/admin/DepartmentModal.tsx (use React Hook Form with Zod validation schema createDepartmentSchema, fields: name required, code required uppercase, description textarea optional, OperatingHoursGrid component with control prop, location/phone/email optional, Submit button calls useCreateDepartment or useUpdateDepartment based on mode, inline validation errors, Cancel closes modal, pre-fill defaultValues if editing)
- [ ] Create app/src/components/admin/DepartmentTable.tsx (accept departments/onEdit/onDeactivate props, render table columns: Department Name sortable, Active Providers badge with count, Total Appointments badge with count, Status green Active or gray Inactive badge, Actions Edit icon and Deactivate icon, onClick Edit calls onEdit with department object, onClick Deactivate calls onDeactivate with validation, filter dropdown for Active/Inactive/All)
- [ ] Create app/src/components/admin/ProviderModal.tsx (React Hook Form, fields: user_id dropdown from GET /api/admin/users filtered by role IN ('doctor', 'staff') without existing provider, specialty input required, license_number optional, department checkboxes from useDepartments with one radio for primary_department, weekly schedule template 7-day grid with start_time/end_time inputs default 09:00-17:00, Submit calls useCreateProvider with {user_id, specialty, department_assignments array, weekly_schedule array}, Cancel closes)
- [ ] Create app/src/components/admin/ProviderScheduleEditor.tsx (use FullCalendar or react-big-calendar in week view, initialView timeGridWeek, hours 6AM-10PM, fetch schedule with useProviderSchedule(providerId), map weekly_schedule to green events, blocked_times to red events, existing_appointments to gray non-editable events with patient initials, selectable true, onSelect callback adds new available slot to local state, eventClick shows context menu Block Time or Remove Slot, save button calls useUpdateProviderSchedule with all schedule entries, display 400 overlap errors)
- [ ] Create app/src/components/admin/BlockedTimeDialog.tsx (small modal form with date picker default today, start_time input, end_time input, reason textarea required, Submit calls useCreateBlockedTime, if 409 ConflictError show alert "Blocked time conflicts with N existing appointments" and list appointments with date/time/patient name, Cancel and "Reschedule Appointments" buttons)
- [ ] Create app/src/components/admin/ProviderTable.tsx (columns: Provider Name, Specialty, Departments comma-separated, Availability Hours formatted "Xh/week", Status Active/Inactive, Actions Edit Schedule icon and Remove icon, filter dropdowns for Department/Specialty/Status above table, onClick Edit Schedule opens ProviderScheduleEditor modal, onClick Remove calls useDeleteProvider with 409 error handling shows AppointmentReassignmentModal)
- [ ] Create app/src/components/admin/AppointmentReassignmentModal.tsx (fetch future appointments with useProviderAppointments(providerId), display list: Appointment Date, Time, Patient Name, Phone, Status, each row has "Reassign to Provider" dropdown populated from useProviders filtered by same department, bulk "Reassign All" dropdown at top, Submit button calls PATCH /api/admin/appointments/bulk-reassign or individual reassignments)
- [ ] Create app/src/pages/DepartmentProviderManagement.tsx (breadcrumb "Admin Dashboard > Department & Provider Management", two-section layout: Departments and Providers, use useDepartments and useProviders hooks with polling, render DepartmentTable with "Add Department" button opens DepartmentModal, render ProviderTable with "Add Provider" button opens ProviderModal, state for modal open/close and selected department/provider for editing, refetchInterval 10000 on queries for NFR-REL01 real-time updates)
- [ ] Modify app/src/pages/index.ts (export DepartmentProviderManagement)
- [ ] Add real-time update notification (compare query data versions, if changed show toast "Schedules updated - click to refresh" with refresh button that calls refetch)
- [ ] Implement WCAG AA accessibility (keyboard navigation Tab through tables and modals, Enter to open/submit, Esc to close modals, ARIA labels on all interactive elements "Edit department {name}", focus management trap in modals, screen reader announcements for toasts)
- [ ] **[UI MANDATORY]** Reference wireframe-SCR-014-department-management.html during implementation (match table layouts, modal structures, calendar editor grid, button placements, spacing)
- [ ] **[UI MANDATORY]** Validate implementation against wireframe at breakpoints 375px, 768px, 1440px (mobile responsive tables, tablet layout, desktop full tables and calendar)
- [ ] Write comprehensive tests (test department table renders and CRUD operations, test operating hours grid validation, test deactivation warning with future appointments, test provider table and CRUD, test schedule editor drag-to-select, test blocked time conflict detection, test appointment reassignment workflow, test polling updates with refetchInterval, test keyboard navigation Tab/Enter/Esc, test accessibility with axe-core)
