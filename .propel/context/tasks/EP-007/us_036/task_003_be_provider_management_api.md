...
- [ ] Create server/src/validators/providerValidators.ts (Zod schemas: createProviderSchema with user_id number, specialty string max 100, license_number optional, department_assignments array of objects {department_id number, primary_department boolean}, weekly_schedule array of {day_of_week number 0-6, start_time regex /^\d{2}:\d{2}$/, end_time regex /^\d{2}:\d{2}$/}, updateProviderSchema as createProviderSchema.partial(), scheduleEntrySchema, blockedTimeSchema with blocked_date date, start_time, end_time, reason string)
- [ ] Create server/src/services/providerService.ts file
- [ ] Implement getAllProviders function (SQL SELECT with JOINs: provider_profiles, users, provider_departments, departments, provider_schedules, STRING_AGG for department names, SUM for total_weekly_hours from schedules where is_available=TRUE, WHERE filters for department/specialty/is_active, GROUP BY provider and user id, ORDER BY last_name first_name, LIMIT OFFSET pagination, return {providers, total, page, limit})
- [ ] Implement createProvider function (validate user exists with role IN ('doctor', 'staff') else throw 400 BadRequestError, check no existing provider_profiles for user_id else throw 409 ConflictError, BEGIN TRANSACTION, INSERT provider_profiles with user_id/specialty/license_number, INSERT provider_departments for each department assignment, INSERT provider_schedules for each weekly_schedule entry, COMMIT, INSERT audit_log action='provider_created', invalidate Redis 'providers:*' and 'appointment-availability:*', return provider with department names)
- [ ] Implement updateProvider function (UPDATE provider_profiles SET specialty/license_number, if department_assignments provided DELETE FROM provider_departments WHERE provider_id then INSERT new assignments, if schedule updates use separate updateProviderSchedule function, INSERT audit_log action='provider_updated', invalidate Redis cache, return updated provider)
- [ ] Implement getProviderSchedule function (SELECT from provider_schedules WHERE provider_id ORDER BY day_of_week start_time, SELECT from provider_blocked_times WHERE provider_id AND blocked_date >= CURRENT_DATE ORDER BY blocked_date, SELECT existing appointments from appointments WHERE provider_id AND appointment_date >= CURRENT_DATE AND status NOT IN ('cancelled', 'completed'), return {weekly_schedule, blocked_times, existing_appointments})
- [ ] Implement updateProviderSchedule function (validate scheduleEntries array, call detectScheduleOverlap to check for overlaps within same day, if hasOverlap throw 400 ValidationError with conflicting entries, BEGIN TRANSACTION, DELETE FROM provider_schedules WHERE provider_id, INSERT batch of new schedule entries, COMMIT, INSERT audit_log action='schedule_updated', invalidate Redis cache, return updated schedule)
- [ ] Implement detectScheduleOverlap helper function (group schedules by day_of_week, for each day sort by start_time, check adjacent entries: if end_time > next.start_time return {hasOverlap: true, conflicts: [entry1, entry2]}, return {hasOverlap: false, conflicts: []})
- [ ] Implement createBlockedTime function (validate blocked_date/start_time/end_time/reason, SELECT conflicts from appointments WHERE provider_id AND appointment_date=blocked_date AND appointment_time >= start_time AND appointment_time < end_time AND status NOT IN ('cancelled', 'completed'), if conflicts.length > 0 throw 409 ConflictError with {has_conflicts: true, appointments} and message 'Blocked time conflicts with existing appointments', else INSERT provider_blocked_times with created_by_user_id=adminId, INSERT audit_log, invalidate cache, return blocked time)
- [ ] Implement deleteProvider function (SELECT COUNT and JSON_AGG appointments WHERE provider_id AND appointment_date >= CURRENT_DATE AND status NOT IN ('cancelled', 'completed'), if count > 0 throw 409 ConflictError with {has_future_appointments: true, appointment_count, appointments} requiring reassignment, else BEGIN TRANSACTION, UPDATE appointments SET is_reassignment_required=TRUE WHERE provider_id for historic records, CASCADE DELETE provider_blocked_times/provider_schedules/provider_departments/provider_profiles, COMMIT, INSERT audit_log action='provider_deleted', invalidate cache, return {message: 'Provider deleted successfully'})
- [ ] Implement getProviderAppointments function (SELECT appointments with patient details: appointment_id, date, time, patient first/last name, phone, status WHERE provider_id AND appointment_date >= CURRENT_DATE AND status NOT IN ('cancelled', 'completed') ORDER BY appointment_date, return appointments array for reassignment workflow)
- [ ] Create server/src/controllers/providerController.ts file (implement handlers: getProviders, createProvider, updateProvider, getProvider, deleteProvider, getProviderSchedule, updateProviderSchedule, createBlockedTime, getProviderAppointments, all with req/res/next, parse params/body, call service functions, catch errors and return appropriate status codes with error messages)
- [ ] Create server/src/routes/providerRoutes.ts (Express Router, import authMiddleware and rbacMiddleware, routes: GET /providers with query params, POST /providers, GET /providers/:id, PUT /providers/:id, DELETE /providers/:id, GET /providers/:id/schedule, POST /providers/:id/schedule, POST /providers/:id/blocked-times, GET /providers/:id/appointments, apply authMiddleware and rbacMiddleware(['admin']) to all routes)
- [ ] Modify server/src/routes/index.ts (import providerRoutes, app.use('/api/admin', providerRoutes))
- [ ] Write comprehensive tests (test getAllProviders with filters, test createProvider success and validation errors, test user role validation doctor/staff only, test duplicate provider 409, test department assignments multiple departments, test schedule creation, test updateProviderSchedule overlap validation returns 400, test createBlockedTime conflict detection returns 409 with appointments, test deleteProvider with future appointments returns 409, test getProviderAppointments for reassignment, test Redis cache invalidation, test audit logging, test admin-only access with RBAC)
