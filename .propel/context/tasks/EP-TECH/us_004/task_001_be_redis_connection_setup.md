...
- [ ] Unit tests pass (Redis client mocking tests)
- [ ] Integration tests pass (actual Upstash Redis connection tests)
- [ ] ioredis installed: `npm list ioredis` shows version 5.x
- [ ] Upstash account created: Console shows active Redis database
- [ ] .env file has Redis credentials: REDIS_URL and REDIS_TOKEN set
- [ ] Server starts and connects to Redis: Console logs "Redis connected to Upstash"
- [ ] TLS encryption enabled: Connection uses rediss:// protocol
- [ ] PING command works: redisClient.ping() returns "PONG"
- [ ] Health endpoint works: `curl http://localhost:3001/api/health/redis` returns JSON with status
- [ ] Connection events logged: Connect, reconnecting, error events logged to console
- [ ] Retry strategy works: Disconnect from Upstash → verify 3 retry attempts with exponential backoff
- [ ] Fallback to database: Set invalid Redis URL → verify app starts, logs fallback warning
- [ ] isRedisAvailable flag: Check flag value when Redis connected (true) vs disconnected (false)
- [ ] Graceful degradation: Redis down → API requests still work (slower, using database)
- [ ] Auto-reconnection: Temporarily disconnect Redis → verify automatic reconnection after 8s
- [ ] No crashes: Redis connection errors don't crash the server

## Implementation Checklist
- [ ] Sign up for Upstash account at https://upstash.com
- [ ] Create Redis database (Free tier: 10K commands/day, 256MB storage)
- [ ] Copy connection URL (rediss://...) and token from Upstash console
- [ ] Install ioredis: `npm install ioredis`
- [ ] Create server/src/config/redis.ts with RedisConfig interface
- [ ] Configure ioredis options: host, port, password (token), tls: { rejectUnauthorized: true }
- [ ] Add retryStrategy: (times) => times > 3 ? null : Math.min(times * 1000, 4000)
- [ ] Create server/src/utils/redisClient.ts with singleton pattern
- [ ] Initialize Redis client: const redis = new Redis(redisConfig)
- [ ] Add connection event handlers: redis.on('connect', () => { isRedisAvailable = true; log('Connected') })
- [ ] Add error handler: redis.on('error', (err) => { isRedisAvailable = false; log.error(err) })
- [ ] Add reconnecting handler: redis.on('reconnecting', () => log.warn('Reconnecting...'))
- [ ] Export redisClient and isRedisAvailable flag
- [ ] Create server/src/utils/redisHealthCheck.ts with async testConnection()
- [ ] Implement PING test: const result = await redisClient.ping(); return result === 'PONG'
- [ ] Wrap in try/catch: catch errors, set isRedisAvailable = false, return false
- [ ] Create server/src/middleware/redisAvailable.ts
- [ ] Check isRedisAvailable flag, attach req.redis = isRedisAvailable ? redisClient : null
- [ ] Create server/src/types/redis.types.ts with RedisConfig, CacheKey, CacheOptions interfaces
- [ ] Update server/.env.example with REDIS_URL, REDIS_TOKEN, REDIS_TLS=true
- [ ] Modify server/src/server.ts: import redisHealthCheck, call await redisHealthCheck() (non-blocking, log result)
- [ ] Modify server/src/app.ts: add GET /api/health/redis route
- [ ] Implement health endpoint: const start = Date.now(); await redis.ping(); const latency = Date.now() - start; return { status: 'ok', redis: 'connected', latency: `${latency}ms` }
- [ ] Add error handling in health endpoint: catch errors, return { status: 'degraded', redis: 'unavailable', fallback: 'database' }
- [ ] Test connection with valid credentials: npm run dev → verify "Redis connected" logged
- [ ] Test connection with invalid credentials: set wrong token → verify error logged, app continues
- [ ] Test health endpoint: curl /api/health/redis → verify latency < 200ms for Upstash
- [ ] Test retry strategy: Pause Upstash database → verify 3 retry attempts, then fallback
- [ ] Document Upstash setup in REDIS_SETUP.md (sign up, create database, get credentials)
- [ ] Add troubleshooting section: TLS errors, connection timeout, authentication failed
- [ ] Test with Upstash free tier limits: Verify 10K commands/day sufficient for development
